...
export default function Report({ state = "create", report }) {
  ...
  const [short_description, setShort_description] = useState(
    report_data.short_description
  );
  const [long_description, setLong_description] = useState(
    report_data.long_description
  );
  const [start, setStart] = useState(
    report_data.start.substring(0, report_data.start.length - 1)
  );
  const [stop, setStop] = useState(
    report_data.stop.substring(0, report_data.stop.length - 1)
  );
  ...
  const saveReport = async (event) => {
    ...
    let submitResult =
      originState == "create"
        ? await AddReport(token, payload)
        : await EditReport(token, report_data.report_id, payload);
    if (
      submitResult?.message == "Report added" ||
      submitResult?.message == "Report updated"
    ) {
      setFormState("view");
      setOriginState("view");
      ...
      <form className="form component" onSubmit={saveReport} id="report-form">
        ...
        <fieldset name="basic information">
          ...
          <input
            ...
            onChange={(event) => setShort_description(event.target.value)}
            disabled={formState == "view"}
          />
        </fieldset>
        <fieldset name="event details">
          ...
          <div className="flex-field">
            <div>
              ...
              >
                ...
                {systems.map((sys) => (
                  <option
                    key={`${sys.system_id}_${sys.system_name}`}
                    value={sys.system_id}
                  >
                    {sys.system_name}
                  </option>
            ...
            <div>
              ...
              <select
                id="opscap"
                value={opscap}
                onChange={(event) => setOpscap(event.target.value)}
                disabled={formState == "view"}
              >
            ...
            <div>
              ...
              <select
                id="syscap"
                value={syscap}
                onChange={(event) => setSyscap(event.target.value)}
                disabled={formState == "view"}
              >
                <option value="N/A">N/A</option>
                <option value="FMC">FMC</option>
          ...
          <textarea
            ...
            onChange={(event) => setLong_description(event.target.value)}
            disabled={formState == "view"}
          />
        </fieldset>
        <fieldset name="cause and fix">
          ...
          <input
            ...
            name="fix_action"
            value={fix_action}
            onChange={(event) => setFix_action(event.target.value)}
            disabled={formState == "view"}
          />
